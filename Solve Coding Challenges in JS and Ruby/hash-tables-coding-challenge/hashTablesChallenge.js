...
function hashTable(arr) {
  let table = Array.from({ length: 11 }, () => []);

  for (let num of arr) {
    const index = Math.abs(num) % 11;
    table[index].push(num);
  }
...
const sampleInput = [12, 24, 125, 5, 91, 1106, 2, 1021, 29, 3536, 10];
console.log("Input:", sampleInput);
console.log("Output:", hashTable(sampleInput));
