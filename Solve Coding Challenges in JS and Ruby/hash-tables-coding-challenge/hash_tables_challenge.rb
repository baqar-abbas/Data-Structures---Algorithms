...
# // Let's say you have a set of integer keys which map to values and you want to create a structure that can find and modify the values in constant time. If your set of keys was just a small range of integers, a HashMap would not be necessary since you could use a simple array to store the values, with the index values of the array as 'keys':
...
def hash_table(arr)
# Create hash table with 11 buckets (index 0 to 10)
  table = Array.new(11) { [] }

  # Hash each number into the table
  arr.each do |num|
    index = num.abs % 11  # Always use absolute value
    table[index] << num
  end
        # Flatten the table while preserving order
  table.flatten
end

# Test with the sample input
sample_input = [12, 24, 125, 5, 91, 1106, 2, 1021, 29, 3536, 10]
puts "Input: #{sample_input}"
puts "Output: #{hash_table(sample_input)}"

# Test with the second sample
second_input = [2, 341, 73, 8265, 234004, 602, 7400000, 200000000]
puts "\nInput: #{second_input}"
puts "Output: #{hash_table(second_input)}"